...
#include "pres_bargraph1_vo.h"  // Put this in graphvisualizations.h potentially
...
void Graphvisualizations::plot_pres_vs_type(QCustomPlot* customPlot, Pres_BarGraph1_VO* bargraph_vo)
{
    QCPBarsGroup *group = new QCPBarsGroup(customPlot);
    QCPBars *bars = new QCPBars(customPlot->xAxis, customPlot->yAxis);
    ...
    // Add each component, with the colour changing based on index
    for (int i = 0; i < bargraph_vo->presTypes.size(); i ++)
    {
        bars = new QCPBars(customPlot->xAxis, customPlot->yAxis);
        customPlot->addPlottable(bars);
        QVector<double> y_component = yvalues.at(i);
    ...
    //Title = first name in the list
    customPlot->plotLayout()->insertRow(0);
    char * x = &bargraph_vo->presenter[0];           // Set the name of the first author, can be modified later to select for a certain author
    QString str = x;
