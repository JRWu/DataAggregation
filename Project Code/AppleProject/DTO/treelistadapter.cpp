...
{
    if(t == PUBLICATION){
        treePath.push_back("Publications");
        ...
        treePath.push_back("Grants/Clinical Trials");
        treePath.push_back(csvline->at(1).getField()); //Start Date
        treePath.push_back(csvline->at(3).getField()); //Name
        treePath.push_back(csvline->at(5).getField()); //Type

        bool peer = (csvline->at(7).getField() == "TRUE");
        bool industry = (csvline->at(8).getField() == "TRUE");
        if(peer){
        ...
        treeValues.push_back(stod(csvline->at(12).getField())); //Total Ammount
    }
    else if(t == PRESENTATION){
        treePath.push_back("Presentations");
        ...
        treePath.push_back("Teaching");
        treePath.push_back(csvline->at(1).getField()); //Start Date
        treePath.push_back(csvline->at(3).getField()); //Name
        treePath.push_back(csvline->at(5).getField()); //Program

        treeValues.push_back(stod(csvline->at(10).getField()));
        try{
            treeValues.push_back(stod(csvline->at(11).getField()));
        }
